 ...
 */
const items={"menu-position":{localStorage:"tablerMenuPosition",default:"top"},"menu-behavior":{localStorage:"tablerMenuBehavior",default:"sticky"},"container-layout":{localStorage:"tablerContainerLayout",default:"boxed"}},config={};for(const[t,e]of Object.entries(items)){const i=localStorage.getItem(e.localStorage);config[t]=i||e.default}const parseUrl=()=>{const t=window.location.search.substring(1).split("&");for(let e=0;e<t.length;e++){const i=t[e].split("="),n=i[0],s=i[1];items[n]&&(localStorage.setItem(items[n].localStorage,s),config[n]=s)}},toggleFormControls=t=>{for(const[e,i]of Object.entries(items)){const i=t.querySelector(`[name="settings-${e}"][value="${config[e]}"]`);i&&(i.checked=!0)}},submitForm=t=>{for(const[e,i]of Object.entries(items)){const n=t.querySelector(`[name="settings-${e}"]:checked`).value;localStorage.setItem(i.localStorage,n),config[e]=n}window.dispatchEvent(new Event("resize")),new bootstrap.Offcanvas(t).hide()};parseUrl();const form=document.querySelector("#offcanvasSettings");form&&(form.addEventListener("submit",function(t){t.preventDefault(),submitForm(form)}),toggleFormControls(form)),
/*!
